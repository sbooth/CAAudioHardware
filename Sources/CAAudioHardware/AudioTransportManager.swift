...
public class AudioTransportManager: AudioPlugIn {
	...
	public class func transportManagers() throws -> [AudioTransportManager] {
		return try AudioSystemObject.instance.getProperty(PropertyAddress(kAudioHardwarePropertyTransportManagerList), elementType: AudioObjectID.self).map { try AudioObject.make($0) as! AudioTransportManager }
	}
	...
	public class func makeTransportManager(forBundleID bundleID: String) throws -> AudioTransportManager? {
		...
		return (try AudioObject.make(objectID) as! AudioTransportManager)
	}
...
extension AudioTransportManager {
	...
	func createEndpointDevice(composition: [AnyHashable: Any]) throws -> AudioEndpointDevice {
		...
		return try AudioObject.make( getProperty(PropertyAddress(kAudioTransportManagerCreateEndPointDevice), type: AudioObjectID.self, qualifier: PropertyQualifier(&qualifier))) as! AudioEndpointDevice
	}
	...
	public func endpointList() throws -> [AudioEndpoint] {
		return try getProperty(PropertyAddress(kAudioTransportManagerPropertyEndPointList), elementType: AudioObjectID.self).map { try AudioObject.make($0) as! AudioEndpoint }
	}
	...
	public func endpoint(forUID uid: String) throws -> AudioEndpoint? {
		...
		return (try AudioObject.make(endpointObjectID) as! AudioEndpoint)
	}
