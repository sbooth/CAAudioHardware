...
extension AudioTransportManager {
	...
	public func whenSelectorChanges(_ selector: AudioObjectSelector<AudioTransportManager>, perform block: PropertyChangeNotificationBlock?, on queue: DispatchQueue? = .global(qos: .background)) throws {
		try whenPropertyChanges(PropertyAddress(PropertySelector(selector.rawValue)), perform: block, on: queue)
	}
}
