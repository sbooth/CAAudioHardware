...
public class AudioTap: AudioObject {
	...
	public static var taps: [AudioTap] {
		get throws {
			try getAudioObjectPropertyData(objectID: .systemObject, property: PropertyAddress(kAudioHardwarePropertyTapList)).map { AudioTap($0) }
		}
	}
	...
	public static func makeTap(forUID uid: String) throws -> AudioTap? {
		...
		let objectID: AudioObjectID = try getAudioObjectPropertyData(objectID: .systemObject, property: PropertyAddress(kAudioHardwarePropertyTranslateUIDToTap), qualifier: PropertyQualifier(&qualifier))
