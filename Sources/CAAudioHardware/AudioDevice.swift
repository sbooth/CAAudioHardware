...
public class AudioDevice: AudioObject {
	...
	public static var devices: [AudioDevice] {
		get throws {
			try getAudioObjectPropertyData(objectID: .systemObject, property: PropertyAddress(kAudioHardwarePropertyDevices)).map { try makeAudioDevice($0) }
		}
	}
	...
	public static var defaultInputDevice: AudioDevice {
		get throws {
			try makeAudioDevice(getAudioObjectPropertyData(objectID: .systemObject, property: PropertyAddress(kAudioHardwarePropertyDefaultInputDevice)))
		}
	}
	...
	public static var defaultOutputDevice: AudioDevice {
		get throws {
			try makeAudioDevice(getAudioObjectPropertyData(objectID: .systemObject, property: PropertyAddress(kAudioHardwarePropertyDefaultOutputDevice)))
		}
	}
	...
	public static var defaultSystemOutputDevice: AudioDevice {
		get throws {
			try makeAudioDevice(getAudioObjectPropertyData(objectID: .systemObject, property: PropertyAddress(kAudioHardwarePropertyDefaultSystemOutputDevice)))
		}
	}
	...
	public static func makeDevice(forUID uid: String) throws -> AudioDevice? {
		...
		let objectID: AudioObjectID = try getAudioObjectPropertyData(objectID: .systemObject, property: PropertyAddress(kAudioHardwarePropertyTranslateUIDToDevice), qualifier: PropertyQualifier(&qualifier))
...
extension AudioDevice {
	...
	public func preferredChannelLayout(inScope scope: PropertyScope) throws -> AudioChannelLayoutWrapper {
		...
		let dataSize = try audioObjectPropertyDataSize(objectID: objectID, property: property)
		let mem = UnsafeMutablePointer<UInt8>.allocate(capacity: dataSize)
		do {
			try readAudioObjectPropertyData(objectID: objectID, property: property, into: mem, size: dataSize)
		} catch let error {
			mem.deallocate()
			throw error
		}
	...
	public func setPreferredChannelLayout(_ value: UnsafePointer<AudioChannelLayout>, inScope scope: PropertyScope) throws {
		...
		try writeAudioObjectPropertyData(objectID: objectID, property: PropertyAddress(PropertySelector(kAudioDevicePropertyPreferredChannelLayout), scope: scope), from: value, size: dataSize)
	}
}
...
extension AudioDevice {
	...
	public func setIOProcStreamUsage(_ value: UnsafePointer<AudioHardwareIOProcStreamUsage>, inScope scope: PropertyScope) throws {
		...
		try writeAudioObjectPropertyData(objectID: objectID, property: PropertyAddress(PropertySelector(kAudioDevicePropertyIOProcStreamUsage), scope: scope), from: value, size: dataSize)
	}
	...
	public func ioThreadOSWorkgroup(inScope scope: PropertyScope = .global) throws -> WorkGroup {
		...
		try readAudioObjectPropertyData(objectID: objectID, property: PropertyAddress(PropertySelector(kAudioDevicePropertyIOThreadOSWorkgroup), scope: scope), into: &value)
